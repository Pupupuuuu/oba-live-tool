...
import { error, kuaishou_group } from '#/constants'
import { LiveControlElementFinder } from '../LiveControlElementFinder'

export class KuaishouGroupControlElementFinder extends LiveControlElementFinder {
  public async getPopUpButtonFromGoodsItem(
    item: ElementHandle<SVGElement | HTMLElement>,
  ): Promise<ElementHandle<HTMLElement | SVGElement>> {
    const button = await item.$(kuaishou_group.selectors.goodsItem.POPUP_BUTTON)
    if (!button) {
  ...
  public async getIdFromGoodsItem(
    ...
    const idInput = await item.$(kuaishou_group.selectors.goodsItem.ID)
  ...
  > {
    const items = await this.page.$$(kuaishou_group.selectors.GOODS_ITEM)
    // 过滤掉置顶的正在弹窗商品（因为没有序号）
  ...
  > | null> {
    return this.page.$(kuaishou_group.selectors.GOODS_ITEMS_WRAPPER)
  }

  public getCommentTextarea(): Promise<
    ElementHandle<SVGElement | HTMLElement> | Locator | null
  > {
    return this.page.$(kuaishou_group.selectors.commentInput.TEXTAREA)
  }
  ...
  > | null> {
    const submitButton = await this.page.$(
      kuaishou_group.selectors.commentInput.SUBMIT_BUTTON,
    )
    if (!submitButton) {
  ...
  public getPinTopLabel(): Promise<ElementHandle<
    SVGElement | HTMLElement
  > | null> {
    return this.page.$(kuaishou_group.selectors.commentInput.PIN_TOP_LABEL)
  }
}
